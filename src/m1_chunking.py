...
def _split_by_size(text: str, size: int) -> list[str]:
    """Chia text thành các đoạn ≤ size, ưu tiên cắt theo paragraph."""
    parts = []
    for para in text.split("\n\n"):
        ...
        if len(para) <= size:
            ...
            for i in range(0, len(para), size):
                parts.append(para[i:i + size].strip())
    merged, current = [], ""
    for p in parts:
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        for cp in _split_by_size(parent.text, child_size):
            children.append(Chunk(text=cp, metadata={**metadata, "chunk_type": "child"},
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    chunks, current_header, content = [], "", []
    for sec in sections:
        sec = sec.strip()
        if not sec:
            continue
        if header_re.match(sec):
            if content:
                chunks.append(Chunk(
                    text=(current_header + "\n\n" + "\n\n".join(content)).strip(),
    ...
    if content or not chunks:
        chunks.append(Chunk(
            text=(current_header + "\n\n" + "\n\n".join(content)).strip() if content else text.strip(),
            metadata={**metadata, "section": current_header, "strategy": "structure"},
        ))
